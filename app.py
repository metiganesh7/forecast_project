import streamlit as stimport pandas as pdimport numpy as npimport plotly.express as pximport plotly.graph_objects as go

from datetime import timedelta

Models

from statsmodels.tsa.statespace.sarimax import SARIMAXfrom xgboost import XGBRegressorfrom sklearn.metrics import mean_squared_errorfrom sklearn.model_selection import TimeSeriesSplitfrom prophet import Prophet
...
st.set_page_config(page_title="AI Forecasting System",page_icon="📈",layout="wide")
...
DATE_COLUMN = "Date"STATE_COLUMN = "State"SALES_COLUMN = "Total"
...
selected_state = st.sidebar.selectbox("📍 Select State",sorted(df[STATE_COLUMN].unique()))

min_date = df[DATE_COLUMN].min()max_date = df[DATE_COLUMN].max()
...
filtered_df["lag_1"] = filtered_df[SALES_COLUMN].shift(1)filtered_df["lag_7"] = filtered_df[SALES_COLUMN].shift(7)filtered_df["lag_30"] = filtered_df[SALES_COLUMN].shift(30)
...
fig = px.line(filtered_df,x=DATE_COLUMN,y=SALES_COLUMN,title=f"{selected_state} Historical Sales")
...
X_train = X[]X_test = X[split_index:]

y_train = y[]y_test = y[split_index:]
...
comparison_df = comparison_df.sort_values("RMSE Score")
...
col2.metric("Forecast Period",f"{forecast_weeks} Weeks")
...
st.code("""GET /forecast

Response:

{"state": "California","best_model": "XGBoost","forecast_weeks": 8,"forecast": [120, 135, 140]}""",language="json")
...
✅ ARIMA Forecasting✅ SARIMA Forecasting✅ Facebook Prophet✅ XGBoost Forecasting✅ LSTM Forecasting✅ Lag Feature Engineering✅ Rolling Mean & Standard Deviation✅ Trend & Seasonality Detection✅ Missing Value Handling✅ Time Series Validation✅ Interactive Dashboard✅ Forecast Download✅ REST API Architecture
...
st.markdown("🚀 AI Powered Time Series Forecasting System using Streamlit")
